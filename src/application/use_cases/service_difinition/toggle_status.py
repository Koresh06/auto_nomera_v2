...
class ToggleServiceStatusUseCase(
    UseCase[ToggleServiceStatusCommand, ServiceDefinitionDTO]
):
    service_repo: ServiceDefinitionRepository
    transaction_manager: TransactionManager

    async def __call__(
        self, command: ToggleServiceStatusCommand
        ...
        logger.info(
            f"[ToggleServiceStatus] service_id={service.id} status={service.is_active}"
        )
